...
with torch.no_grad():
    ...
    for nSimul in range(args.n_simulations):
        ...
        if currArgs.deconv_iterations > 0:
            ...
            deconv_net,proj_net,forward_net,_ = XLFMDeconv(OTF, img_to_deconv_net, currArgs.deconv_iterations, device=device_deconv, all_in_device=args.deconv_gpu, max_allowed=args.deconv_limit)
            end.record()
            torch.cuda.synchronize()
