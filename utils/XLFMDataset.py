...
class XLFMDataset(data.Dataset):
    def __init__(self, data_path, lenslet_coords_path, subimage_shape, img_shape=[2160,2160], images_to_use=None, lenslets_offset=0, divisor=4, isTiff=False,
     load_vols=True, load_all=False, load_sparse=False, n_frames_net=1, temporal_shifts=[]):
        # Load lenslets coordinates
        self.lenslet_coords = get_lenslet_centers(lenslet_coords_path) + torch.tensor(lenslets_offset)
        self.n_lenslets = self.lenslet_coords.shape[0]
        self.data_path = data_path
        self.n_frames = min([get_number_of_frames(data_path), n_frames_net])
        self.load_vols = load_vols
        self.load_all = load_all
        self.load_sparse = load_sparse
        self.temporal_shifts = temporal_shifts
        ...
        if isTiff:
            ...
            # dataset = Image.open(imgs_path)
            self.img_dataset = imread(imgs_path)
            n_frames,h,w = np.shape(self.img_dataset)

            if self.load_sparse:
                self.img_dataset_sparse = imread(imgs_path_sparse)

            if images_to_use is None:
                images_to_use = list(range(n_frames))
            self.n_images = min(len(images_to_use), n_frames)

            self.all_files = sorted(glob.glob(vols_path))[:self.n_images]

            if self.load_sparse:
                vols_path_sparse = data_path + '/XLFM_stack_S/*.tif'
                self.all_files= sorted(glob.glob(vols_path_sparse))[:self.n_images]
            if load_vols:
                # read single volume
                currVol = self.read_tiff_stack(self.all_files[0])
                odd_size = [currVol.shape[0]//self.divisor,currVol.shape[1]//self.divisor]
            ...
            # Grab patches
            if load_all:
                self.stacked_views = torch.zeros(self.n_images, self.n_lenslets, self.subimage_shape[0], self.subimage_shape[1],dtype=torch.float16)
            else:
                self.stacked_views = torch.zeros(1, self.n_lenslets, self.subimage_shape[0], self.subimage_shape[1],dtype=torch.float16)
            ...
            for nImg in range(self.n_images):
                ...
                if self.load_all:
                    # self.img_dataset.seek(nImg)
                    image = torch.from_numpy(np.array(self.img_dataset[nImg,:,:]).astype(np.float16)).type(torch.float16)

                    image = self.pad_img_to_min(image)
                    self.stacked_views[nImg,...] = self.extract_views(image, self.lenslet_coords, self.subimage_shape)
                    
                    if self.load_sparse:
                        image = torch.from_numpy(np.array(self.img_dataset_sparse[nImg,:,:]).astype(np.float16)).type(torch.float16)
                        image = self.pad_img_to_min(image)
                        stacked_views_sparse[nImg,...] = self.extract_views(image, self.lenslet_coords, self.subimage_shape)

            if self.load_sparse:
                self.stacked_views = torch.cat((self.stacked_views.unsqueeze(-1), stacked_views_sparse.unsqueeze(-1)), dim=4)

        else:
            self.all_files = glob.glob(data_path + "/*.nrrd")
            if images_to_use is not None:
                assert(len(images_to_use) < len(self.all_files))
                self.all_files = [self.all_files[i] for i in images_to_use]
            
            self.n_images = len(self.all_files)
           
            ## Preload images
            # Select sample
            currVolInfo = nrrd.read(self.all_files[0])
            currVol = torch.from_numpy(currVolInfo[0].astype(float))
            currVol = currVol[0:currVol.shape[0]//self.divisor,0:currVol.shape[1]//self.divisor]
            currFileName = os.path.basename(self.all_files[0])[:-4]

            half_volume_shape = [currVol.shape[0]//2,currVol.shape[1]//2]
            # Grab patches
            self.stacked_views = torch.zeros(self.n_images, self.n_lenslets, self.subimage_shape[0], self.subimage_shape[1])
            self.vols = torch.zeros(self.n_images, currVol.shape[2], currVol.shape[0], currVol.shape[1])
            for nImg in range(self.n_images):

                currVolInfo = nrrd.read(self.all_files[nImg])[0]
                self.vols[nImg,:,:,:] = torch.from_numpy(currVolInfo.astype(float)).permute(2,0,1)\
                    [:,currVolInfo.shape[0]//2-half_volume_shape[0]-1:currVolInfo.shape[0]//2+half_volume_shape[0],\
                        currVolInfo.shape[1]//2-half_volume_shape[1]-1:currVolInfo.shape[1]//2+half_volume_shape[1]]
                
                
                currFileName = os.path.basename(self.all_files[nImg])[:-4]
                self.all_files[nImg] = currFileName
                currImg = Image.open(self.data_path + '/XLFM_Image_' + currFileName + "tif")
                image = ToTensor()(currImg)
                self.stacked_views[nImg,...] = extract_views(image[0,...], self.lenslet_coords, self.subimage_shape)
    ...
    def __len__(self):
        'Denotes the total number of samples'
        return int(self.n_images / self.n_frames)
    ...
    def get_max(self):
        ...
        if self.load_sparse:
            ...
            return self.stacked_views.float().max().type(self.stacked_views.type()),self.vols.float().max().type(self.vols.type())
    ...
    def __getitem__(self, index):
        ...
        if self.n_frames == 1: 
            ...
            if index+temporal_shifts_ixs[-1]>self.n_images:
                ...
                newIndex = self.n_frames*int(index//self.n_frames)
                index = newIndex
            indices = [index + i for i in temporal_shifts_ixs] #list(range(index,index+self.n_frames))
            views_out = self.stacked_views[indices,...]
            nDims = list(range(2,len(views_out.shape)))
            views_out = views_out.permute(1,0,*nDims)
        if self.load_vols is False:
    ...
    def extract_views(image, lenslet_coords, subimage_shape):
        ...
        half_subimg_shape = [subimage_shape[0]//2,subimage_shape[1]//2]
        n_lenslets = lenslet_coords.shape[0]
        stacked_views = torch.zeros(size=[n_lenslets, subimage_shape[0], subimage_shape[1]], device=image.device).type(image.type())
        for nLens in range(n_lenslets):
            # Fetch coordinates
            currCoords = lenslet_coords[nLens,:]
            # Grab patches
            ...
            lower_bounds = [max(lower_bounds[kk],0) for kk in range(2)]
            currPatch = image[lower_bounds[0] : currCoords[1]+half_subimg_shape[0], lower_bounds[1] : currCoords[0]+half_subimg_shape[1]]
            stacked_views[nLens,-currPatch.shape[0]:,-currPatch.shape[1]:] = currPatch
